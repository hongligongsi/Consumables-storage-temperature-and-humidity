...
主界面 EC11 单击开关 LED，双击开启/关闭手动强排，长按开启/关闭系统；旋转切换耗材。点击屏幕上的当前耗材名进入耗材设置页：旋转修改选中值，单击依次选择最低/最高仓温、最低/最高自动排气风速、打印结束排气风速和排气时长，双击选择上一项，长按写入 NVS 并返回。温度上下限至少相差 5℃，自动风速上下限会互相约束，打印结束风速范围为 0–100%，排气时长范围为 0–1800 秒（0 表示关闭）。REST `POST /api/profile` 除 `index` 外也接受 `minC`、`maxC`、`fanMin`、`fanMax`、`postFan`、`postSeconds`，校验通过后立即持久化。

主界面底部触摸按钮依次为 PIR 状态指示、系统启停、提前预热、灯光和系统设置；三个控制行的开关及左右耗材箭头也可直接触摸。进入 17 项系统设置后由 EC11 旋转选择，单击切换或进入数值编辑，长按保存返回。系统设置包括中英文、按键音、亮度/休眠、打印保持亮屏、编码器方向、PIR 延时、灯光/蜂鸣联动、发热板限流/风扇 PWM/温度保护、四线电阻触摸两点校准和恢复出厂。休眠后的首次触摸只唤醒屏幕；恢复出厂只删除固件拥有的设置与耗材键，不清除未知键，因此预留的注册码数据会保留。
