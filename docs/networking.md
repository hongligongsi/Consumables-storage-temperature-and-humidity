...
手机连上后自动跳转定制配网页（**不支持 5 GHz**）。页面提供附近 WiFi 扫描
列表、信号百分比、开放/加密状态、SSID 手动输入、密码显示/隐藏、连接状态与失败
原因，以及「保存并连接」按钮。门户启动后先进行异步预扫描并缓存结果，正常页面
浏览不会把扫描等待放进 50 ms 温控循环；3 分钟无操作自动关闭并重试。
